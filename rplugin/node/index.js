  ...
  try {
    ...
    data.forEach((file) => {
      ...
      nodes.push({
        ...
        title: file_path.split('/')[file_path.split('/').length - 1].split('.')[0],
        level: 0,
        pos: 0,
        properties: {},
        tags: [],
...
module.exports = (plugin) => {
  ...
  function init() {
    plugin.nvim.lua('return vim.g.org_roam_ui_root').then((org_roam_ui_root) => {
      // 如果配置了这个地址，那么直接启动 org roam ui 服务器
      const { exec } = require('child_process')
      exec(`npm run dev --prefix ${org_roam_ui_root}`)
      setTimeout(() => {
  ...
  plugin.registerCommand('GetLatestGraphData', [plugin.nvim.buffer, getGraphData])
